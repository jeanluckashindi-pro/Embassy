...
import { CalendarDays, Newspaper, Search, ArrowRight, ExternalLink, MapPin, Mail, Tag } from "lucide-react";
...
export function ActualitesPage({ SiteHeader, SiteFooter }) {
  ...
  const articles = DEFAULT_NEWS;

  const filteredArticles = articles.filter((item) => {
    const matchesSearch = item.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
      item.description.toLowerCase().includes(searchTerm.toLowerCase());
    const matchesTab = selectedTab === "all" || item.category.toLowerCase() === selectedTab;
  ...
  return (
    <main className="site-shell bg-white dark:bg-[#161717] text-slate-900 dark:text-[#fafad6] min-h-screen">
      ...
      {/* Header Banner */}
      <section className="py-14 bg-blue-900 dark:bg-[#1d1f1f] text-white border-b border-blue-950 dark:border-[#2d2e2e]">
        <div className="container max-w-5xl">
          <Badge variant="gold" className="mb-3">Presse & Communiqués</Badge>
          <h1 className="text-3xl sm:text-5xl font-extrabold text-white dark:text-[#fafad6] tracking-tight">
            Actualités de l'Ambassade
          </h1>
          <p className="mt-3 text-blue-100 dark:text-[#a7a8a8] text-sm sm:text-base max-w-2xl">
                ...
                <div className="p-5 pt-0">
                  <Dialog>
                    <DialogTrigger asChild>
                      <button className="w-full bg-slate-100 hover:bg-blue-800 hover:text-white text-slate-800 font-bold py-2.5 px-4 rounded-xl text-xs flex items-center justify-between transition-colors cursor-pointer border border-slate-200">
                        <span>Lire l'article</span>
                        <ArrowRight className="h-3.5 w-3.5" />
                      </button>
                    </DialogTrigger>
                    <DialogContent className="bg-white border border-slate-200 text-slate-900 sm:max-w-xl">
                      <DialogHeader>
                        ...
                        <DialogDescription className="text-xs text-slate-500">{item.source}</DialogDescription>
                      </DialogHeader>
                      <div className="space-y-4 py-3 text-xs text-slate-700 leading-relaxed">
                        <p className="font-semibold text-slate-900">{item.description}</p>
                        <p>{item.content}</p>
                        <div className="p-3 rounded-xl bg-blue-50 border border-blue-100 text-[11px] text-blue-900">
