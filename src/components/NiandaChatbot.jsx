    ...
    <>
      ...
      {!isOpen && (
        <div className="fixed bottom-4 right-4 sm:bottom-5 sm:right-5 z-[999]">
          <motion.button
            ...
            className="flex items-center gap-2 sm:gap-3 bg-blue-900 hover:bg-blue-800 text-white font-semibold py-2 px-3 sm:py-3 sm:px-4 rounded-full shadow-2xl border-2 border-amber-400 cursor-pointer group"
            aria-label="Ouvrir l'assistant Nianda"
          >
            <div className="relative flex items-center justify-center">
              <div className="h-8 w-8 sm:h-9 sm:w-9 rounded-full bg-amber-400 text-slate-950 flex items-center justify-center font-bold text-xs sm:text-sm shadow-md">
                <Bot className="h-4 w-4 sm:h-5 sm:w-5 text-slate-950" />
              </div>
            ...
            <div className="text-left pr-0.5 sm:pr-1">
              <span className="text-[11px] sm:text-xs font-bold text-amber-300 block leading-tight">Nianda</span>
              <span className="text-[9px] sm:text-[10px] text-slate-200 hidden sm:block font-normal">Orientation Consulaire</span>
            </div>
            <MessageSquare className="h-4 w-4 sm:h-5 sm:w-5 text-amber-300 ml-0.5 sm:ml-1 group-hover:rotate-12 transition-transform" />
          </motion.button>
        </div>
      )}
      ...
      {isOpen && isMinimized && (
        <div className="fixed bottom-4 right-4 sm:bottom-5 sm:right-5 z-[999]">
          ...
          >
            <div className="h-6 w-6 sm:h-7 sm:w-7 rounded-lg bg-amber-400 text-slate-950 flex items-center justify-center font-bold text-xs">
              N
            </div>
            ...
            <Maximize2 className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-amber-400 ml-1 sm:ml-2" />
          </motion.button>
        </div>
      )}
          ...
          >
            ...
            <div className="bg-blue-900 dark:bg-[#1D1F1F] p-3.5 sm:p-4 border-b border-blue-950 dark:border-[#2D2E2E] flex items-center justify-between text-white relative">
              <div className="flex items-center gap-2.5 sm:gap-3">
                <div className="relative">
                  <div className="h-9 w-9 sm:h-10 sm:w-10 rounded-xl bg-amber-400 text-slate-950 flex items-center justify-center font-bold shadow-md">
                ...
                <div>
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-[9px] sm:text-[10px] bg-amber-400/20 text-amber-300 font-semibold px-2 py-0.5 rounded-full border border-amber-400/30">
                      RDC Consulat
                    </span>
                  </div>
              ...
              <div className="absolute bottom-0 left-0 right-0 h-[2px] bg-gradient-to-r from-blue-400 via-amber-400 to-red-500" />
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-3 sm:p-4 space-y-3.5 sm:space-y-4 text-xs bg-slate-50 dark:bg-[#1D1F1F]">
              {messages.map((msg) => (
                ...
                >
                  {msg.sender === "bot" && (
                    <div className="h-6 w-6 sm:h-7 sm:w-7 rounded-lg bg-amber-400 text-slate-950 flex items-center justify-center shrink-0 mt-0.5 font-bold text-xs shadow-xs">
                      N
                    </div>
                  )}
                      ...
                      {msg.actionLink && (
                        <div className="mt-2.5 pt-2 border-t border-slate-200 dark:border-[#2D2E2E]">
                          <a
                            ...
                            className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-amber-400 hover:bg-amber-300 text-slate-950 font-bold text-xs transition-colors"
                          >
                      ...
                      <div className="flex flex-wrap gap-1 sm:gap-1.5 pt-1">
                        {msg.actions.map((act, i) => {
                          ...
                          return (
                            ...
                            >
                              {IconComponent && <IconComponent className="h-3 w-3 sm:h-3.5 sm:w-3.5 text-blue-700 dark:text-amber-400 shrink-0" />}
                              <span>{act.label}</span>
                            </button>
                          );
                        })}
              ...
              {isTyping && (
                <div className="flex gap-2.5 items-center text-slate-500 dark:text-[#A7A8A8] text-xs">
                  <div className="h-6 w-6 sm:h-7 sm:w-7 rounded-lg bg-amber-400 text-slate-950 flex items-center justify-center shrink-0 font-bold text-xs">
                    N
                  </div>
                  <div className="bg-white dark:bg-[#161717] border border-slate-200 dark:border-[#2D2E2E] p-2.5 sm:p-3 rounded-2xl rounded-tl-none flex items-center gap-1.5">
                    ...
                    <span className="h-2 w-2 rounded-full bg-red-500 animate-bounce delay-300" />
                  </div>
                </div>
              )}
